...
function CatalogV2Page() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['products-v2', q, page],
    queryFn: () =>
      callApi<ProductV2ListResponse>(
        `/v2/products?q=${encodeURIComponent(q)}&page=${page}&page_size=${PAGE_SIZE}`
      ),
  ...
  return (
    <main className="mx-auto max-w-6xl p-6">
      <div className="mb-6 flex flex-col gap-1">
        ...
        <p className="text-sm text-muted-foreground">Extended product catalog</p>
      </div>
